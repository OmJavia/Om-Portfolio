...
export default function Home() {
  ...
  const skills = [
    ['Python', SiPython], ['JavaScript', SiJavascript], ['Computer Vision', FaEye], ['NLP & LLMs', FaLanguage], ['RAG', FaProjectDiagram], ['Deep Learning', FaBrain], ['PyTorch', SiPytorch], ['OpenCV', SiOpencv], ['MySQL', SiMysql], ['MongoDB', SiMongodb], ['Docker', SiDocker], ['AWS', SiAmazonaws], ['Git', SiGit], ['Postman', SiPostman]
  ];
  ...
  return (
    <Layout>
      <Head>
        ...
        <meta property="og:image" content="https://omjavia.vercel.app/og-image.png" />
        ...
        <meta name="twitter:image" content="https://omjavia.vercel.app/og-image.png" />
      ...
      <main className="mx-auto max-w-7xl px-5 sm:px-8">
        <motion.section id="home" variants={stagger} initial="hidden" animate="show" className="grid min-h-[82vh] items-center gap-12 py-16 lg:grid-cols-[1.05fr_.95fr]">
          <div>
            ...
            <motion.div variants={reveal} className="mt-8 flex flex-wrap gap-3">
              <MagneticButton href="/contact" className="rounded-2xl bg-neural px-6 py-3 text-sm font-semibold text-white shadow-neural">Hire me <span aria-hidden>↗</span></MagneticButton>
              <MagneticButton href="https://cal.com/Om-Javia" target="_blank" rel="noreferrer" className="rounded-2xl border border-border bg-surface px-6 py-3 text-sm font-semibold text-white">Schedule a Meeting <span aria-hidden>↗</span></MagneticButton>
            </motion.div>
            <motion.div variants={reveal}><AiAssistantModal /></motion.div>
            <motion.div variants={reveal} className="mt-7 flex gap-2">
          ...
          <motion.div variants={reveal} className="relative">
            <div className="absolute -inset-10 rounded-full bg-neural/10 blur-3xl" />
            <div className="relative overflow-hidden rounded-[2rem] border border-border bg-surface/80 p-2 shadow-2xl backdrop-blur-xl">
              <Image src={front} width={900} height={900} alt="Om Javia AI engineer workspace illustration" className="h-auto w-full rounded-[1.5rem] object-cover" priority sizes="(max-width: 1024px) 100vw, 50vw" />
              <div className="absolute bottom-6 left-6 right-6 rounded-2xl border border-white/10 bg-black/60 p-4 font-mono text-[10px] text-zinc-400 backdrop-blur-xl"><span className="text-electric">system</span> / intelligent products / shipped</div>
            </div>
          </motion.div>
        </motion.section>

        <section id="experience" className="scroll-mt-24 py-20">
          <motion.div variants={reveal} initial="hidden" whileInView="show" viewport={{ once: true, amount: .3 }}><span className="font-mono text-[10px] uppercase tracking-[.2em] text-neural">Experience</span><h2 className="mt-2 font-heading text-3xl font-bold text-white sm:text-5xl">Building systems that move metrics.</h2></motion.div>
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[['33','%','Product / traffic impact'],['87.4','%','Mars terrain accuracy'],['185','+','Players reached with AI'],['5','+','International markets']].map(([value,suffix,label]) => <SpotlightCard key={label} className="p-6"><div className="font-mono text-4xl font-bold text-white"><AnimatedCounter value={value} suffix={suffix} /></div><p className="mt-2 text-sm text-zinc-500">{label}</p></SpotlightCard>)}
          </div>
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            {[['Metashot','Business Associate / AI Product','International product launches, technical operations and AI-enabled sports products.'],['ISRO SAC','Research Intern','Mars terrain classification, annotated satellite imagery and obstacle-aware rover navigation.'],['Apoliums Infotech','Software Development Engineer','Scalable web applications across inventory, food delivery, hotel management and e-commerce.']].map(([company, role, text]) => <SpotlightCard key={company} className="p-6"><span className="font-mono text-[10px] uppercase tracking-widest text-electric">{role}</span><h3 className="mt-2 font-heading text-xl font-bold text-white">{company}</h3><p className="mt-3 text-sm leading-6 text-zinc-400">{text}</p></SpotlightCard>)}
          </div>
        </section>

        <section id="stack" className="scroll-mt-24 py-20">
          <div className="text-center"><span className="font-mono text-[10px] uppercase tracking-[.2em] text-neural">Capabilities</span><h2 className="mt-2 font-heading text-3xl font-bold text-white sm:text-5xl">The full <span className="text-neural">AI stack.</span></h2><p className="mx-auto mt-4 max-w-2xl text-zinc-400">From architecting neural networks to deploying scalable production environments. End-to-end AI design and engineering.</p></div>
          <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {[[FaBrain,'Computer Vision','Real-time vision systems that track, segment, and understand the physical world.',['OpenCV','YOLO']],[FaProjectDiagram,'NLP & LLMs','RAG pipelines, autonomous agents, and language-model workflows.',['LangChain','RAG']],[FaDatabase,'Data Science','Extracting actionable intelligence and predictive patterns from complex data streams.',['Pandas','Scikit']],[FaCloudUploadAlt,'AI Deployment','Production-ready cloud microservices engineered for low latency and availability.',['AWS','Docker']]].map(([Icon,title,text,tags]) => <SpotlightCard key={title} className="flex min-h-64 flex-col p-7 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-electric/10 text-xl text-electric"><Icon /></div><h3 className="mt-5 font-heading text-lg font-bold text-white">{title}</h3><p className="mt-2 flex-1 text-sm leading-6 text-zinc-500">{text}</p><div className="mt-5 flex justify-center gap-2">{tags.map(t => <span key={t} className="rounded-full border border-border px-2.5 py-1 font-mono text-[9px] text-zinc-400">{t}</span>)}</div></SpotlightCard>)}
          </div>
        </section>
        ...
        <section id="projects" className="scroll-mt-24 py-20">
          ...
          <motion.div layout className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{visibleProjects.map(project => <motion.div layout key={project.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .4 }}><ProjectCard {...project} /></motion.div>)}</motion.div>
        </section>
